...
export const content: ToolLocaleContent = {
  ...
  seo: [
    ...
    { type: 'paragraph', html: 'Esta secção explica com mais detalhe a geometria usada pelo simulador. O raio de Schwarzschild cresce com a massa e, na solução ideal de Schwarzschild, representa uma superfície de coordenadas associada ao horizonte de eventos. A esfera de fotões fica mais afastada e corresponde a órbitas circulares instáveis: uma pequena perturbação pode alterar rapidamente o destino da luz. A dilatação temporal apresentada depende do observador escolhido e não deve ser confundida com o tempo próprio de um relógio local. Altere primeiro a massa e depois a distância para comparar escalas sem misturar os efeitos. O modelo não inclui rotação, carga elétrica, matéria envolvente, disco de acreção, campos magnéticos, radiação ou forças de maré de um objeto concreto. Serve para criar intuição sobre a relatividade geral e distinguir uma definição matemática de uma observação astronómica. Para uma fonte real seriam necessários dados de massa, rotação, ambiente e trajetória, além de uma metodologia publicada. A página não fornece previsões operacionais nem instruções para aproximar-se de um objeto compacto. Registe as hipóteses e compare uma variável de cada vez.' },
    { type: 'paragraph', html: 'Também pode alterar a posição do observador e comparar como muda a apresentação do tempo. Este exemplo ajuda a distinguir uma representação por coordenadas da medição de um relógio local. Num objeto real, a rotação e a matéria envolvente influenciam os percursos da luz e o sinal observado; um modelo esférico simples não descreve todas as observações. Registe cada parâmetro e repita o cálculo nas mesmas condições para compreender a origem e os limites do resultado.' },
    { type: 'paragraph', html: 'Compare várias execuções independentes e altere apenas um parâmetro de cada vez. Assim pode distinguir um efeito geométrico do modelo de uma consequência da escolha do observador. As fórmulas ajudam a criar intuição, mas a interpretação depende sempre do referencial e das condições iniciais.' },
    { type: 'paragraph', html: 'Use as mesmas definições para repetir a comparação e altere depois um único parâmetro. Assim fica mais claro o que resulta da geometria e o que depende do referencial escolhido. O simulador é um modelo educativo, não uma descrição completa de um buraco negro observado.' },
  ],
  faq,
  bibliography,
  howTo,

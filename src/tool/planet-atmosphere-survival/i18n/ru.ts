import { bibliography } from '../bibliography';
import type { ToolLocaleContent } from '../../../types';

const slug = 'kalkulyator-vyzhivaniya-v-atmosfere-planety';
const title = 'Калькулятор выживания в атмосфере планеты';
const description = 'Как долго вы сможете прожить без скафандра на Марсе, Венере, Титане, Юпитере или Эвересте? Этот интерактивный калькулятор оценивает время выживания незащищённого человека с учётом давления, температуры, состава газа и экстремальных условий.';

const howTo = [
  {
    name: 'Выберите цель для загрузки атмосферных данных',
    text: 'Выберите Марс, Венеру, Титан, Юпитер или вершину Эвереста, чтобы автоматически применить реалистичные значения давления, температуры и газового состава для этой среды.',
  },
  {
    name: 'Регулируйте параметры для поиска переломных моментов',
    text: 'Используйте ползунки давления, температуры и концентрации газов, чтобы увидеть, какой фактор станет смертельным в первую очередь. Даже небольшие изменения могут критически сместить ограничивающий фактор.',
  },
  {
    name: 'Анализируйте время выживания и слабое звено',
    text: 'Таймер показывает расчетное время до возникновения критического биологического стресса, а индикатор ограничивающего фактора указывает на наиболее актуальную угрозу.',
  },
  {
    name: 'Сравнивайте риски с помощью визуальных карт',
    text: 'Радиальные диаграммы и графики временной шкалы наглядно показывают вклад давления, гипоксии, температуры и токсичности в общий профиль риска.',
  },
];

const faq = [
  {
    question: 'Может ли человек выжить на Марсе без скафандра?',
    answer: 'Нет. Атмосферное давление на Марсе составляет менее 1% от земного, что значительно ниже предела Армстронга, при котором жидкости организма начинают закипать. Потеря сознания наступает через несколько секунд, а смерть - через несколько минут из-за гипоксии и декомпрессии.',
  },
  {
    question: 'Почему атмосферное давление критично для жизни?',
    answer: 'Ниже предела Армстронга (около 6,3 кПа) точка кипения воды падает до температуры человеческого тела. Кроме того, при низком давлении парциальное давление кислорода становится недостаточным для поддержания насыщения крови, что делает дыхание невозможным даже при наличии кислорода.',
  },
  {
    question: 'Какая из представленных сред наиболее благоприятна?',
    answer: 'Вершина Эвереста является наиболее благоприятной, хотя пребывание там без акклиматизации крайне опасно. Титан наименее враждебен среди планет, так как его давление близко к земному, однако экстремальный криогенный холод и отсутствие кислорода делают его смертельным.',
  },
  {
    question: 'Что опаснее на Венере: жара или давление?',
    answer: 'Оба фактора экстремальны. Давление на поверхности в 92 раза выше земного, а температура превышает 460°C. Эти угрозы действуют одновременно, делая выживание невозможным практически мгновенно.',
  },
  {
    question: 'Почему время выживания на Титане дольше, чем на Марсе?',
    answer: 'На Титане плотная атмосфера, поэтому давление не является мгновенной угрозой. Основными ограничивающими факторами являются экстремальный холод (-180°C) и отсутствие кислорода, что дает человеку чуть больше времени, чем при декомпрессионном шоке в условиях марсианского вакуума.',
  },
  {
    question: 'Каковы главные причины смерти в космосе без защиты?',
    answer: 'Ключевые факторы: потеря давления (эбуллизм и гипоксия), экстремальная температура, воздействие токсичных газов и конвективная теплопотеря, вызванная ветрами.',
  },
  {
    question: 'Подходит ли этот инструмент для планирования реальных космических миссий?',
    answer: 'Нет. Это образовательная модель, использующая упрощенные биологические пороги. Реальный анализ декомпрессии и гипоксии требует экспертной оценки.',
  },
];

export const content: ToolLocaleContent = {
  slug,
  title,
  description,
  ui: {
    riskMap: 'Карта атмосферного риска',
    timeline: 'Временная шкала биологического риска',
    controls: 'Параметры атмосферы',
    destination: 'Цель',
    pressure: 'Давление',
    temperature: 'Температура',
    oxygen: 'Кислород',
    co2: 'Углекислый газ',
    limitingFactor: 'Ограничивающий фактор',
    verdict: 'Вердикт',
    exposureSummary: 'Сводка воздействия',
    atmosphericModel: 'Модель воздействия',
    survivalEnvelope: 'Предел выживания',
    survival: 'Выживание',
    mode: 'Режим',
    metric: 'Метрическая',
    imperial: 'Имперская',
    unitSystem: 'Система единиц',
    vitalStress: 'Жизненный стресс',
    timeLabel: 'Время',
    estimatedSurvival: 'Расчётное выживание',
    hazardPressure: 'Давление',
    hazardTemperature: 'Температура',
    hazardOxygen: 'Гипоксия',
    hazardToxicity: 'Токсичность',
    hazardWind: 'Ветер',
    presetMars: 'Марс',
    presetVenus: 'Поверхность Венеры',
    presetTitan: 'Титан',
    presetJupiter: 'Облачный слой Юпитера',
    presetEverest: 'Земля, вершина Эвереста',
    noteMars: 'Почти вакуум, экстремальный холод и отсутствие кислорода.',
    noteVenus: 'Раздавливающее давление и температура, плавящая свинец.',
    noteTitan: 'Плотный азотный воздух, криогенный холод.',
    noteJupiter: 'Богатая водородом среда, ураганные ветры.',
    noteEverest: 'Серьезная гипоксия и холод, опасные для неподготовленных.',
    verdictSeconds: 'Секунды',
    verdictMinutes: 'Минуты',
    verdictHours: 'Часы',
    verdictExtended: 'Риск длительного воздействия',
  },
  seo: [
    {
      type: 'title',
      text: 'Калькулятор выживания в атмосфере планеты: Марс, Венера, Титан или Юпитер?',
      level: 2,
    },
    {
      type: 'paragraph',
      html: 'Сколько вы проживете без скафандра на других планетах? Этот калькулятор оценивает время выживания человека без защиты на Марсе, Венере, Титане, Юпитере и Эвересте, моделируя шесть основных угроз: давление, кислород, температуру, CO2, токсичность и ветер.',
    },
    {
      type: 'paragraph',
      html: 'Результаты представляют собой обучающую оценку. Инструмент помогает понять, почему разные миры опасны по-разному. Марс губит вакуумом за секунды, Венера сочетает раздавливающее давление и жару, а Титан замораживает в отсутствие кислорода.',
    },
    {
      type: 'title',
      text: 'Какая планета обладает наиболее благоприятной атмосферой?',
      level: 3,
    },
    {
      type: 'paragraph',
      html: 'Среди всех небесных тел вершина Эвереста наиболее переносима. Из других планет Титан имеет самое комфортное давление, но губит от отсутствия кислорода и криогенного холода.',
    },
    {
      type: 'title',
      text: 'Как каждая угроза влияет на организм',
      level: 3,
    },
    {
      type: 'list',
      items: [
        '<strong>Низкое давление:</strong> ниже предела Армстронга (6,3 кПа) жидкости организма закипают. Это главный фактор на Марсе.',
        '<strong>Высокое давление:</strong> сжимает дыхательные газы и повреждает легкие.',
        '<strong>Парциальное давление кислорода:</strong> гипоксия наступает даже при 21% O2, если общее давление слишком низкое.',
        '<strong>Высокая температура:</strong> вызывает мгновенный тепловой шок и разрушение белков (Венера).',
        '<strong>Низкая температура:</strong> криогенный холод Титана (-180°C) вызывает мгновенное обморожение.',
        '<strong>Токсичность CO2:</strong> высокое содержание углекислого газа приводит к удушью.',
        '<strong>Токсическая химия:</strong> соединения серы, аммиак и метан разъедают ткани.',
        '<strong>Ветер:</strong> ураганные ветры ускоряют потерю тепла.',
      ],
    },
    {
      type: 'title',
      text: 'Марс: почему низкое давление убивает первым',
      level: 3,
    },
    {
      type: 'paragraph',
      html: 'Давление на Марсе (0,6 кПа) намного ниже предела Армстронга. Потеря сознания наступает менее чем за 15 секунд из-за эбуллизма и острой гипоксии.',
    },
    {
      type: 'title',
      text: 'Венера: сочетание экстремального давления и жары',
      level: 3,
    },
    {
      type: 'paragraph',
      html: 'Давление в 92 атмосферы и температура 462°C действуют одновременно, делая выживание невозможным буквально за секунды.',
    },
    {
      type: 'title',
      text: 'Титан: самое комфортное давление в Солнечной системе за пределами Земли',
      level: 3,
    },
    {
      type: 'paragraph',
      html: 'Давление на Титане (147 кПа) вполне переносимо человеком, однако отсутствие кислорода и холод в -180°C быстро приводят к гибели.',
    },
    {
      type: 'title',
      text: 'Как интерпретировать таймер выживания и карту рисков',
      level: 3,
    },
    {
      type: 'paragraph',
      html: 'Таймер показывает расчетное время до настуления летального исхода, а диаграмма наглядно демонстрирует вклад каждого опасного фактора.',
    },
    {
      type: 'table',
      headers: ['Планета', 'Опасные факторы', 'Главная угроза', 'Что должен компенсировать скафандр'],
      rows: [
        ['Марс', 'Вакуум, гипоксия, холод', 'Давление < предела Армстронга', 'Герметичный скафандр, кислород, термозащита'],
        ['Венера', 'Давление 92 атм, жара 462°C', 'Давление и температура', 'Бронированный скафандр с охлаждением'],
        ['Титан', 'Нет кислорода, холод -179°C', 'Гипоксия и холод', 'Кислородная маска, мощный обогрев'],
        ['Юпитер', 'Нет кислорода, ураганный ветер', 'Гипоксия', 'Автономная система дыхания'],
        ['Эверест', 'Гипоксия, холод', 'Низкое давление кислорода', 'Кислородный баллон, теплая одежда'],
      ],
    },
    {
      type: 'title',
      text: 'Что нужно для выживания без скафандра?',
      level: 3,
    },
    {
      type: 'paragraph',
      html: 'Ни одна планета в Солнечной системе не позволяет человеку выжить без специального оборудования.',
    },
    {
      type: 'list',
      items: [
        '<strong>Используйте для обучения:</strong> наглядно сравнивайте условия различных планет.',
        '<strong>Используйте для сравнения:</strong> оцените разницу между Титаном и Венерой.',
        '<strong>Используйте для дискуссий:</strong> обсудите требования к терраформированию.',
        '<strong>Не используйте для реальных расчетов:</strong> это концептуальная научная модель.',
      ],
    },
    {
      type: 'title',
      text: 'Важные ограничения и образовательная цель',
      level: 3,
    },
    {
      type: 'paragraph',
      html: 'Реальное выживание зависит от множества факторов здоровье, одежду, уровень физической нагрузки и индивидуальные особенности организма. Этот инструмент использует упрощенные биологические пороги.',
    },
    {
      type: 'paragraph',
      html: 'Результат следует читать как сравнение условий, а не как медицинский прогноз. Сначала изучите давление и состав атмосферы, затем отдельно оцените температуру, углекислый газ и скорость ветра. Даже короткое воздействие может быть опасным, поэтому модель не заменяет скафандр, герметичную кабину, расчёт миссии или рекомендации специалиста по безопасности. Условия на поверхности могут меняться по высоте, времени суток и месту. Используйте одинаковые параметры для учебного сравнения планет и отмечайте, какие угрозы определяют итоговую оценку. Такой подход помогает связать числа с физическими причинами: низкое давление ограничивает работу жидкостей организма, отсутствие кислорода нарушает дыхание, а экстремальная температура ускоряет потерю тепла или перегрев. Сценарий на Эвересте приведён для понятного сравнения с земной средой, но и там фактический риск зависит от подготовки и оборудования. Модель также показывает, почему разные угрозы нужно рассматривать вместе. Давление и температура определяют физическую нагрузку, состав воздуха влияет на дыхание, а ветер меняет скорость теплообмена. При интерпретации не складывайте независимые пороги механически: итоговая оценка зависит от того, какая опасность наступает первой и какие допущения использованы в расчёте. Для реальной экспедиции необходимы проверенные данные о месте, запас кислорода, связь и план аварийного возвращения. При сравнении планет полезно фиксировать одну переменную и менять её отдельно, иначе невозможно понять причину изменения оценки. Учитывайте, что шкала времени в калькуляторе отражает выбранные пороги модели, а не точный биологический прогноз. Любой практический сценарий требует независимой проверки атмосферы, температуры и оборудования непосредственно перед работой.',
    },
  ],
  faq,
  bibliography,
  howTo,
  schemas: [
    { '@context': 'https://schema.org', '@type': 'SoftwareApplication', name: title, description, applicationCategory: 'ScientificApplication', operatingSystem: 'Any' },
    { '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: faq.map((item) => ({ '@type': 'Question', name: item.question, acceptedAnswer: { '@type': 'Answer', text: item.answer } })) },
    { '@context': 'https://schema.org', '@type': 'HowTo', name: title, step: howTo.map((step) => ({ '@type': 'HowToStep', name: step.name, text: step.text })) },
  ],
};
